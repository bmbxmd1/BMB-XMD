 ...
 * MakeStore.js
 *
 * Ported from NOVA-XMD's lib/MakeStore.js (originally ES Modules) to
 * CommonJS so it drops into BMB-TECH's existing require()-based code
 * unchanged in behavior.
 *
 * Newer Baileys builds (including the xhclintohn/Baileys#beta-test fork
 * this project uses) no longer export makeInMemoryStore, so this
 * lightweight custom store replaces it: chats/contacts caching with
 * automatic trimming so memory doesn't grow unbounded over a long-running
 * bot process, plus loadMessage()/getMessage() for Baileys' getMessage
 * socket option (used for retry/resend of outgoing messages).
 */
